/**
 * Combined FMEA generation — DBC signals + design spec components in one pass.
 * Deletes all existing AI items once, then inserts both source sets together.
 * Usage: npx tsx scripts/generate-fmea-combined.ts [session_id]
 */
import fs from 'fs'
...
// ── DBC PARSER ────────────────────────────────────────────────────────────────
interface SbwSignal {
  msgName: string; msgId: number; sender: string; sigName: string
  length: number; signed: boolean; factor: number; offset: number
  min: number; max: number; unit: string; receivers: string[]
  comment: string | null; isSbwSender: boolean
}
...
function parseDbcForSbw(text: string): SbwSignal[] {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const sm = rawLine.match(SIG_RE)  // use rawLine to preserve leading spaces
    if (sm && cur) {
      cur.signals.push({
  ...
  for (const msg of messages.values()) {
    ...
    for (const sig of msg.signals) {
      ...
      result.push({ msgName: msg.name, msgId: msg.id, sender: msg.sender, ...sig, comment, isSbwSender })
    }
  }
  return result
}
...
function getHazopWords(dtype: string): string[] {
  if (dtype === 'bool') return ['CORRUPT']
  return ['MORE', 'LESS', 'CORRUPT']
}

function getDefaultSod(hazop: string, dtype: string, isOutput: boolean, sigName: string): { s: number; o: number; d: number } {
  const isCrc      = /Crc|AlvCnt|E2E/i.test(sigName)
  const isCritical = /SBW_(Gear|Park|Target|Actu|Shift|Lever|Pos|Req|Sta)/i.test(sigName)
  const isGear     = /Gear|TCU_Gear|HTCU_Gear/i.test(sigName)
  const isVehSpd   = /VehSpd|Speed|Spd/i.test(sigName)
  if (isCrc)                              return { s: 7, o: 3, d: 4 }
  if (isCritical && isOutput) {
    if (hazop === 'CORRUPT')              return { s: 9, o: 3, d: 4 }
    if (hazop === 'MORE' || hazop === 'LESS') return { s: 8, o: 3, d: 5 }
    return                                       { s: 7, o: 3, d: 5 }
  }
  if (isGear) {
    if (hazop === 'CORRUPT')              return { s: 8, o: 3, d: 4 }
    return                                       { s: 7, o: 4, d: 5 }
  }
  if (isVehSpd) return { s: 7, o: 3, d: 4 }
  if (dtype === 'bool') return isOutput  ? { s: 7, o: 3, d: 5 } : { s: 6, o: 3, d: 5 }
  if (hazop === 'CORRUPT')               return { s: 6, o: 3, d: 5 }
  return { s: 5, o: 3, d: 5 }
}

function buildDbcItem(sig: SbwSignal, hazop: string): DbRow {
  ...
  const rangeStr  = (sig.min !== 0 || sig.max !== 0)
    ...
    : dtype
  const { s, o, d } = getDefaultSod(hazop, dtype, isOutput, sig.sigName)
  const sigDesc   = sig.comment ? ` (${sig.comment})` : ''
  const sigInfo   = `${sig.sigName}${sigDesc} [${rangeStr}]`
  const direction = isOutput ? 'SBW 송신' : `${sig.sender} → SBW 수신`
  const sw_component = isOutput ? `DBC_SBW_TX (${sig.msgName})` : `DBC_SBW_RX (${sig.msgName})`
  ...
  switch (hazop) {
    case 'MORE':
      failure_detail    = `${sigInfo} 신호값이 정상 범위를 초과`
      effect_local      = isOutput ? 'SBW 출력 신호 과도값으로 수신 ECU 오판단' : '과도한 입력값으로 SBW 제어 로직 오동작'
      effect_system     = isOutput ? '수신 ECU에 잘못된 상태 정보 → 시스템 제어 불일치' : '잘못된 입력으로 SBW 변속 제어 오류'
      potential_cause   = `SW 연산 오류 또는 센서 포화로 ${sig.sigName} 값이 최대(${sig.max}) 초과`
      preventive_action = '출력 값 범위 클램핑 및 Range Check 적용'
      detection_action  = '범위 초과 감지 DTC 및 Unit Test 경계값 검증'
      break
    case 'LESS':
      failure_detail    = `${sigInfo} 신호값이 정상 범위 미만으로 송출`
      effect_local      = isOutput ? 'SBW 출력 신호 과소값으로 수신 ECU 제어 부족' : '낮은 입력값으로 SBW 제어 응답 부족'
      effect_system     = isOutput ? '수신 ECU의 필요 제어량 미달 → 기능 부분 손실' : 'SBW 변속 제어 부족으로 운전자 의도 미반영'
      potential_cause   = `센서 고장 또는 접촉불량으로 ${sig.sigName} 값이 최솟값(${sig.min}) 미만`
      preventive_action = '최솟값 경계 클램핑 및 플라우시빌리티 체크 적용'
      detection_action  = '하한 범위 감지 DTC 및 응답 모니터링'
      break
    default: // CORRUPT
      if (/Crc|AlvCnt|E2E/i.test(sig.sigName)) {
        failure_detail    = `${sigInfo} E2E/CRC 보호 신호 값 불일치 또는 변조`
        effect_local      = 'E2E 검증 실패로 해당 메시지 수신 무효화'
        effect_system     = '메시지 무효화로 SBW 제어 기능 일시 중단 또는 Fallback 진입'
        potential_cause   = '전기적 노이즈 또는 버스 충돌로 CRC/AlvCnt 값 오염'
        preventive_action = 'E2E Profile 2 프로토콜 적용 및 CRC 모듈 코드 리뷰'
        detection_action  = 'E2E 오류 카운터 DTC 및 롤링카운터 불일치 감지'
      } else if (dtype === 'bool') {
        failure_detail    = `${sigInfo} bool 신호가 반전 또는 비정상 값 수신`
        effect_local      = 'SBW 상태 판단 로직에 잘못된 플래그 입력'
        effect_system     = isOutput ? '수신 ECU에 잘못된 상태 전달 → 불필요한 제어 조치' : 'SBW 제어 조건 오판단으로 의도치 않은 변속'
        potential_cause   = 'EMI/노이즈에 의한 비트 반전 또는 상태 머신 오류'
        preventive_action = '중복 플래그 검증 및 상태 머신 트랜지션 Plausibility 체크'
        detection_action  = '상태값 이중화 비교 및 유효 범위 검증 DTC'
      } else {
        failure_detail    = `${sigInfo} 신호값이 물리적으로 불가능한 값 또는 비정상 패턴`
        effect_local      = '비정상 입력에 의한 SW 컴포넌트 이상 동작'
        effect_system     = isOutput ? '외부 ECU에 무효 데이터 전달 → 안전 모드 진입' : 'SBW 제어 오류로 기어 포지션 불일치'
        potential_cause   = '전기적 노이즈 또는 송신 ECU SW 버그로 불가능한 값 발생'
        preventive_action = '플라우시빌리티 체크 및 신호 필터링 알고리즘 적용'
        detection_action  = '이상값 감지 DTC 및 연속 비교 모니터링'
      }
  }

  return {
    sw_component,
    function_name: `${direction}: ${sig.sigName}`,
    failure_mode: hazop,
    failure_detail, effect_local, effect_system, potential_cause,
...
const SPEC_ITEMS: DbRow[] = [
  // CstAp_PwrMGT
  { sw_component:'CstAp_PwrMGT', function_name:'CtAp_IgnStaChk — IGN 전압 PowerOn 판단 (V_IGN_OFF=4V, V_IGN_ON=7V)',
    failure_mode:'CORRUPT', failure_detail:'VIgnVolt이 V_IGN_OFF(405U)~V_IGN_ON(762U) 사이에서 진동하여 PowerOnSta chattering 발생',
  ...
  { sw_component:'CstAp_PwrMGT', function_name:'CtAp_BatVoltChk — 배터리 과전압 감지 (V_BAT_OVER=16.5V)',
    failure_mode:'MORE', failure_detail:'배터리 전압이 V_BAT_OVER(1910u) 초과했으나 V_BAT_OVER_TIME_SET(2s) 카운터 초기화 버그로 과전압 플래그 미설정',
  ...
  { sw_component:'CstAp_HapticControlMgt', function_name:'SPI 햅틱 명령 전송 (SELECT_PLAYBACK=9)',
    failure_mode:'CORRUPT', failure_detail:'SPI 노이즈로 햅틱 액추에이터에 SELECT_PLAYBACK(9) 외의 값 전달되어 비정상 진동',
  ...
  { sw_component:'CstAp_DtcMgt', function_name:'DTC Snapshot 데이터 (SNAPSHOT_0xFD50_SIZE=18u) 수집',
    failure_mode:'CORRUPT', failure_detail:'DTC 발생 시점 Snapshot 데이터(18바이트) 수집 중 인터럽트로 일부 바이트가 다른 시점 값으로 혼재',
  ...
  { sw_component:'BswIF_CAN', function_name:'INIT_VALUE 초기값 처리 — VCU_GEAR_POS_STA 초기값(9u)',
    failure_mode:'CORRUPT', failure_detail:'ECU 초기화 시 VCU 기어 포지션 초기값(9u=미정의)이 실제 유효값으로 오처리',
  ...
  { sw_component:'BswIF_NvM', function_name:'CtNvM — SPEC_OPTION_ReadBlock() 초기 읽기',
    ...
    effect_local:'SpecOptinState 초기값 미설정으로 특수 기능 옵션 적용 전 기본값으로 동작', effect_system:'차종 옵션에 따른 SBW 특수 기능 미적용으로 불완전한 기능 제공',
  ...
  { sw_component:'BswIF_Dcm_19_RDTCI', function_name:'DTC BatteryVoltageHigh 상태 설정 (U3003_A3)',
    failure_mode:'CORRUPT', failure_detail:'SetEventStatus(FAILED) 호출 조건이 경쟁 조건으로 PASSED/FAILED 상태가 반전 설정됨',
...
// ── MAIN ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`\n▶ Combined FMEA generation (DBC + 설계사양서)`)
  console.log(`  세션: ${SESSION_ID}`)

  // 1. Load DBC
  const sessionDir = path.join(UPLOAD_DIR, SESSION_ID)
  const dbcDir = path.join(sessionDir, 'dbc_file')
  if (!fs.existsSync(dbcDir) || !fs.readdirSync(dbcDir).length) {
    console.error('❌ DBC 파일이 없습니다.'); process.exit(1)
  }
  const dbcFile = fs.readdirSync(dbcDir)[0]
  const dbcText = fs.readFileSync(path.join(dbcDir, dbcFile), 'utf-8')
  console.log(`✅ DBC loaded: ${dbcFile}`)

  const sbwSignals = parseDbcForSbw(dbcText)
  console.log(`✅ SBW 신호 파싱: ${sbwSignals.length}개`)

  // 2. Generate DBC-based FMEA items
  const dbcItems: DbRow[] = []
  for (const sig of sbwSignals) {
    if (/CCP_REQ_Byte|CCP_RESP_Byte|GST_SBW_Byte/.test(sig.sigName)) continue
    const dtype  = getDataType(sig)
    for (const hazop of getHazopWords(dtype)) {
      dbcItems.push(buildDbcItem(sig, hazop))
    }
  }
  console.log(`✅ DBC 기반 FMEA 항목 생성: ${dbcItems.length}개`)
  console.log(`✅ 설계사양서 기반 FMEA 항목: ${SPEC_ITEMS.length}개`)

  const allRows = [...dbcItems, ...SPEC_ITEMS]
  console.log(`✅ 총 항목: ${allRows.length}개`)
  ...
  try {
    ...
    for (let i = 0; i < allRows.length; i++) {
      ...
      await client.query(
        ...
        [SESSION_ID, item_no, row.sw_component, row.function_name, row.failure_mode,
         ...
         s, o, d, row.preventive_action, row.detection_action, row.confidence_score, ap],
      )
    }
    ...
    const srcDist = { dbc: dbcItems.length, spec: SPEC_ITEMS.length }
    ...
    console.log(`\n✅ DB 삽입 완료: ${allRows.length}개`)
    console.log(`\n[소스별]`)
    console.log(`  DBC 신호 기반      : ${srcDist.dbc}개`)
    console.log(`  설계사양서 기반    : ${srcDist.spec}개`)
    console.log(`\n[AP 분포]`)
    for (const [ap, cnt] of Object.entries(apDist).sort()) console.log(`  ${ap}: ${cnt}개`)
    console.log(`\n[SW 컴포넌트별 (상위 15개)]`)
    const sorted = Object.entries(compDist).sort((a, b) => b[1] - a[1]).slice(0, 15)
    for (const [comp, cnt] of sorted) console.log(`  ${comp.padEnd(35)}: ${cnt}개`)
    console.log(`  ... 총 ${Object.keys(compDist).length}개 컴포넌트`)
  } catch (e) {
