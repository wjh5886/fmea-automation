...
function buildKnowledgeContext(rules: Record<string, unknown>[]): string {
  if (!rules.length) return ''
  const sod     = rules.filter(r => r.content_type === 'sod_rule')
  const missing = rules.filter(r => r.content_type === 'missing_pattern')
  const hints   = rules.filter(r => r.content_type === 'quality_hint')
  const lines: string[] = []
  if (sod.length)     lines.push('◆ SOD 평가 보정 규칙:\n' + sod.map(r => `  - ${r.content}`).join('\n'))
  if (missing.length) lines.push('◆ 누락하지 말아야 할 패턴:\n' + missing.map(r => `  - ${r.content}`).join('\n'))
  if (hints.length)   lines.push('◆ 품질 지침:\n' + hints.map(r => `  - ${r.content}`).join('\n'))
  if (!lines.length) return ''
  return `\n■ [이전 프로젝트 학습 규칙 — 반드시 적용]\n${lines.join('\n')}\n`
}
...
${opts.referenceTable}
${opts.sgSmCtx}${opts.knowledgeCtx}
■ 3. 프로젝트별 Input 데이터
...
export async function POST(req: NextRequest) {
  try {
    ...
    // knowledge 규칙 로드 (quality_score >= 0.5, 최대 15개)
    const knowledgeRows = await query(
      "SELECT id, content, content_type FROM pre_fmea_knowledge WHERE quality_score >= 0.5 ORDER BY usage_count DESC, quality_score DESC LIMIT 15",
      [],
    ).catch(() => [] as Record<string, unknown>[])
    const knowledgeCtx = buildKnowledgeContext(knowledgeRows)
    // 사용된 규칙 usage_count 증가 (fire-and-forget)
    if (knowledgeRows.length) {
      const ids = knowledgeRows.map(r => String(r.id))
      ids.forEach(id => execute(
        'UPDATE pre_fmea_knowledge SET usage_count = usage_count + 1 WHERE id = $1', [id],
      ).catch(() => {}))
    }

    // ICD 모드: ICD 변수 기반 배치 생성으로 분기
    if (mode === 'icd') {
      return handleIcdGenerate(session_id, session, dbcCtx, specCtx, humanItems, sgSmCtx, knowledgeCtx)
    }
    ...
    const userPromptText = buildUserPrompt({ itemName, referenceTable, templateCtx, archCtx, dbcCtx, specCtx, sgSmCtx, knowledgeCtx })
